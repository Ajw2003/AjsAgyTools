...
> `csharp-unity-standards.md`, `python-standards.md`, and `web-js-ts-node-standards.md`.

## Core principles

- **Clarity over cleverness.** If a teammate (or future me, or an agent) needs a comment to
...
- Autonomous read-only discovery (e.g. searching code, inspecting git logs) runs freely without human interruption.
- Destructive commands (force push, hard reset, process kill, recursive deletion) must always be gated with user confirmation.
